            ...
            <div class="col-md-8">
                ...
                <div>
                    ...
                    <p>
                        ...
                        Вместо генерации ленты новостей "на лету", посты предназначенные для ленты новостей конкретного
                        пользователя хранятся в отдельной таблице "feed".<br>
                        У данного метода хранения записей есть свои минусы и плюсы.<br>
                        Благодаря этому лента формируется очень быстро, так как записи в таблице "feed" формируются под
                        конкретного подписчика, но пользователь видит посты в ленте добавленные только после подписки на
                        пользователей.<br>
                    </p>
                </div>
                ...
                <?php if (!Yii::$app->user->isGuest && !empty($feeds)) : ?>
                    ...
                    <?php endforeach; ?>
                <?php elseif (!Yii::$app->user->isGuest && empty($feeds)) : ?>
                    <p>Здесь будут отображаться посты добавленные пользователями после момента вашей подписки на
                        них.</p>
                <?php else: ?>
